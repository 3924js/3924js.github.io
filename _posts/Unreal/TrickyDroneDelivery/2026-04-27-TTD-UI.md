...
캔버스에 텍스트 블록들을 추가해줬다. 왼쪽 위에는 점수, 오른쪽 위에는 남은 상자와 남은 목숨을 표시한다. 단순 텍스트만 띄우는 것으론 의미가 없을 것이다. 이전에 추가해놓은 델리게이트를 이용해 일정시간마다 UI를 업데이트 하도록 해보자. 매 틱마다 업데이트한다면 지금 수준의 간단한 게임은 상관 없겠지만 성능에 꽤나 부하를 줄 수 있을 것이다.

0.1초마다 델리게이트의 broadcast에 의해 UI를 업데이트 하도록 호출된다. 플레이해보면 진행에 따라 숫자가 바뀌는 것을 볼 수 있다.
