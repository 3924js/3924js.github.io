...
또 하나의 방법은 SetActorLocation(), SetActorRotation()으로 Holdable의 위치를 강제로 조작하는 방법이 있다. 충돌도 살아있고 벽을 뚫거나 하지는 않겠지만, 어떤 조건에서
그리고 지금 쓸 방법인 PhysicalHandleComponent/PhysicalConstraintComponent를 사용하는 방법도 있다. 장착할 액터를 줄에 매단것처럼 물리적으로 연결하게 해주는 컴포넌트인데, Handle은 느슨하게 연결되어 장착된 Pawn의 Tick()에서 위치의 업데이트를 해주어야된다. 단순히 SetActorLocation()등의 방법으로 불러오는 것과는 다르게, 물리 연산을 끄지 않아도 되고, 미리 handle에 주어진 damping등의 수치에 따라 물리적으로 자연스럽게 딸려오도록 구현된다. 그에 비해 Constraint방식은 관절에 달린 것처럼 Pawn과 한몸체가 되는 수준으로 단단하게 결합한다.
...
3. 1번과 2번의 조합
	...
	* 지금 구현한 Drone과 Holdable도 여기에 속한다.
